...
}), wr = typeof navigator < "u" ? navigator.hardwareConcurrency ?? 4 : 4, Tr = Math.max(1, Math.floor(wr / 2)), Er = Math.min(8, Tr), Dr = null, Or = 0, H = Object.freeze({
...
async function hl(e, t, n = {}) {
	...
	return await Promise.all(e.map((e, r) => {
		if (!e || typeof e != "object") throw Error(`routeBatch request at index ${r} must be an object with start, end, and mode`);
		let { start: i, end: a, mode: o, costField: s } = e;
		if (et(i, `requests[${r}].start`), et(a, `requests[${r}].end`), typeof o != "string" || o.length === 0) throw Error(`routeBatch request at index ${r} must include a valid mode`);
		return s !== void 0 && ot(s), ml(i, a, o, t, {
			...n,
			costField: s
		});
	}));
